...
/*
arameter "coll_tuned_alltoall_algorithm" (current value: "ignore", data source: default, level: 5 tuner/detail, type: int)
...
void FisherYatesAllToAll(int * arr, unsigned int size, unsigned int seed){
    srand(seed);
        ...
        for (j = i+1 ; j < size ; ++j) {
            ...
            if (idx[i] == rank) {
                ...
                err = ompi_datatype_copy_content_same_ddt (rdtype, rcount, tmp_buffer,
                                                       (char *) rbuf + idx[j] * max_size);
                if (MPI_SUCCESS != err) { goto error_hndl; }

                /* Exchange data with the peer */
                err = MCA_PML_CALL(recv ((char *) rbuf + max_size * idx[j], rcount, rdtype,
                                          idx[j], MCA_COLL_BASE_TAG_ALLTOALL, comm, MPI_STATUS_IGNORE));//, preq++));
                if (MPI_SUCCESS != err) { goto error_hndl; }

                err = MCA_PML_CALL(send ((char *) tmp_buffer,  rcount, rdtype,
                                          idx[j], MCA_COLL_BASE_TAG_ALLTOALL, MCA_PML_BASE_SEND_STANDARD,
                                          comm));//, preq++));
                if (MPI_SUCCESS != err) { goto error_hndl; }
            } else if (idx[j] == rank) {
                /* Copy the data into the temporary buffer */
                err = ompi_datatype_copy_content_same_ddt (rdtype, rcount, tmp_buffer,
                                                       (char *) rbuf + idx[i] * max_size);
                if (MPI_SUCCESS != err) { goto error_hndl; }

                /* Exchange data with the peer */
                err = MCA_PML_CALL(recv ((char *) rbuf + max_size * idx[i], rcount, rdtype,
                                          idx[i], MCA_COLL_BASE_TAG_ALLTOALL, comm, MPI_STATUS_IGNORE));//, preq++));
                if (MPI_SUCCESS != err) { goto error_hndl; }

                err = MCA_PML_CALL(send ((char *) tmp_buffer,  rcount, rdtype,
                                          idx[i], MCA_COLL_BASE_TAG_ALLTOALL, MCA_PML_BASE_SEND_STANDARD,
                                          comm));//, preq++));
                if (MPI_SUCCESS != err) { goto error_hndl; }
            } else {
                continue;
